...
def update_sections_config(general_data):
    ...
    Args:
            general_data (dict): the current version of config.json as dictionary
    """
    sections = {el['section_name'] for story in general_data['data_sources'].values()
...
def init_datastory(user_type, form_data, general_data, user_name=None):
    if user_type == 'polifonia':
        clean_title, clean_section = add_story_to_config(
            'config.json', form_data, general_data)
...
def manage_datastory_data(user_type, general_data, file, section_name, datastory_name):
    ...
    dynamic_elements = []
    position_set = {int(k.split('__')[0])
                    for k, v in form_data.items() if '__' in k}
    color_code_list = []
        ...
        for position in position_set:
            ...
            for k, v in form_data.items():
                if '__' in k and position == int(k.split('__')[0]):
                    component_data = [c for c in components.components if bool(
                        re.search(rf"{c['regex_attr']}", k)) == True][0]
                    ...
                    if "donothing" not in component_data and component_data['value_type'] == "dict":
                        ...
                        key = re.search(
                            rf"{component_data['regex_key']}", k).group(1)
                        if (component_data["postprocess_value"] == "" and "list_value" not in component_data):
                            ...
                            if key in elements_dict[component_data['name']]:
                                if v != 'false':
                                    elements_dict[component_data['name']][key].append(
                                        v)
                            else:
                                if v != 'false':
                                    elements_dict[component_data['name']][key] = [
                                        v]
                        else:
                            elements_dict[component_data['name']][key] = eval(
                                'components.'+component_data["postprocess_value"]+'(v)')
            ...
            filter_queries = sorted(
                filter_queries, key=lambda k: k['position'])
